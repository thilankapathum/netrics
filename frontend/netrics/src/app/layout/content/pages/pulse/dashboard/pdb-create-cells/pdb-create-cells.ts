    ...
    this.dashboardService.createWorstCellsByRatAndAreaType('week', this.areaType()!, this.selectedDate(), this.rat(), this.granularity())    //TODO: Modify week for better arguments
      .subscribe({
        next: (result) => {
          ...
          console.log(err);
          this.alertService.error('Error creating Worst Cells', 'Error', `${err.status} ${err.statusText}`);
          this.creatingWorstCells.set(false);
        }
      });
  }
    ...
    this.statusPollingSub = interval(3000)
      .pipe(
        ...
        next: (result) => {
          ...
          console.log(err);
          this.alertService.error('Error retrieving Worst Cells creation status', 'Error', `${err.status} ${err.statusText}`);
        }
      })
  }
