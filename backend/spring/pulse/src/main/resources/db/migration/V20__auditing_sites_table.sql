-- Add audit columns to 'sites' table to support AuditEntity extension.
...
COMMENT ON COLUMN sites.created_at        IS 'Timestamp when the row was first created.';
COMMENT ON COLUMN sites.created_by        IS 'User ID (JWT sub) who created the row.';
COMMENT ON COLUMN sites.last_modified_at  IS 'Timestamp of the most recent update.';
COMMENT ON COLUMN sites.last_modified_by  IS 'User ID (JWT sub) who last modified the row.';