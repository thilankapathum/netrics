-- V18__add_audit_columns_to_cells.sql
-- Add audit columns to 'cells' table to support AuditEntity extension.
...
-- creator is unknown. You can tighten this to a known migration timestamp.
UPDATE cells
SET created_at = NOW(),
    created_by = 'SYSTEM'
...
COMMENT ON COLUMN cells.created_at        IS 'Timestamp when the row was first created.';
COMMENT ON COLUMN cells.created_by        IS 'User ID (JWT sub) who created the row.';
COMMENT ON COLUMN cells.last_modified_at  IS 'Timestamp of the most recent update.';
COMMENT ON COLUMN cells.last_modified_by  IS 'User ID (JWT sub) who last modified the row.';